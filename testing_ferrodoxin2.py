...
for run in fva_result_dict.keys():
    if fva_result_dict[run][0]['ATPS']['maximum'] - fva_result_dict[run][0]['ATPS']['minimum'] < 10 \
        and fva_result_dict[run][0]['ATPS']['minimum'] > 0:
        atps_dict[run] = fva_result_dict[run]

# for run in atps_dict.keys():
#     if atps_dict[run][0]['Eha_FSLASH_Ehb']['maximum'] > 0:
#         print run
